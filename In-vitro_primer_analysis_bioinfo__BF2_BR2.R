...
#I only want certain samples from my list actually having the BF2/BR2 primers,
#in this case samples 21-30, 31 & 32.

# Forward reads
fnFs <- sort(list.files(
  path,
  pattern = "CAW-25-36-(01[1-9]|020|031|032).*R1_001.fastq.gz$",
  full.names = TRUE
))
...
fnRs <- sort(list.files(
  path,
  pattern = "CAW-25-36-(01[1-9]|020|031|032).*R2_001.fastq.gz$",
  full.names = TRUE
))
...
path.cut <- file.path(path, "BF2BR2_CO1/cutadapt")
...
jpeg(file="BF2BR2_CO1/BF2BR2_CO1.Quality.Plot.F.jpg",res=300, width=15, height=8, units="in")
fwd_qual_plots
dev.off()

jpeg(file="BF2BR2_CO1/BF2BR2_CO1.Quality.Plot.R.jpg",res=300, width=15, height=8, units="in")
rev_qual_plots
dev.off()
...
out <- filterAndTrim(cutFs, filtpathF, cutRs, filtpathR,
                     truncLen=c(220, 220), maxEE=c(4,6), truncQ=2, maxN=0, rm.phix=TRUE,
...
jpeg(file="BF2BR2_CO1/BF2BR2_CO1.Error.F.Plot.jpg",res=300, width=15, height=8, units="in")
plotErrors(errF, nominalQ=TRUE)
dev.off()

jpeg(file="BF2BR2_CO1/BF2BR2_CO1.Error.R.Plot.jpg",res=300, width=15, height=8, units="in")
...
write.csv(track,"BF2BR2_CO1/BF2BR2_CO1.track.csv")
...
saveRDS(seqtab, "BF2BR2_CO1/BF2BR2_CO1.seqtab.rds")
...
jpeg(file="BF2BR2_CO1/BF2BR2_CO1.Chim.Dist.Plot.jpg",res=300, width=30, height=8, units="in")
g
dev.off()
...
seqtab2 <- seqtab[,nchar(colnames(seqtab)) %in% seq(418,425)]

seqtab.nochim <- removeBimeraDenovo(seqtab2, multithread=TRUE, verbose=TRUE)
saveRDS(seqtab.nochim, "BF2BR2_CO1/BF2BR2_CO1.seqtab.nochim.rds")
...
write.csv(track2, "BF2BR2_CO1/BF2BR2_CO1.track2.csv")  
...
seqtab.nochim <- readRDS("BF2BR2_CO1/BF2BR2_CO1.seqtab.nochim.rds")
...
head(asv_fasta)
write.table(asv_fasta, "BF2BR2_CO1/BF2BR2_CO1_ASV_seq_IDs_and_sequences.tsv", sep="\t", quote=F, col.names=NA)
write(asv_fasta, "BF2BR2_CO1/BF2BR2_CO1.nochim_ASVs.fa")
