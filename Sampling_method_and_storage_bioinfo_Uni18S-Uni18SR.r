...
dadaFs <- dada(derepFs, err=errF, multithread=cores, pool="pseudo")
dadaRs <- dada(derepRs, err=errR, multithread=cores, pool="pseudo")
...
cat("\nRemoving chimeras\n")
seqtab.nochim1 <- removeBimeraDenovo(seqtab1, method="consensus", multithread=cores, verbose=TRUE)
dim(seqtab.nochim1)
saveRDS(seqtab.nochim1, file = paste0(path_results,"/seqtab.nochim_26_07.rds"))

# Inspect distribution of sequence lengths
df = as.data.frame(table(nchar(getSequences(seqtab.nochim1))))
...
out_5 = data.frame(rownames(seqtab.nochim1),rowSums(seqtab.nochim1)) %>% setNames(c("sample_id","nonchim"))
...
seqtab.nochim2 <- readRDS("/srv/users/Joelle/CAW-25-36/Uni18S_COTS_pipeline/DADA2_18S_results/seqtab.nochim.rds")
