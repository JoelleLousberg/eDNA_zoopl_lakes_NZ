...
setwd("C:/Users/joelle.lousberg/OneDrive - Cawthron/Documents/Chapter_2/R_step2/BF2BR1_CO1")
...
##BOLD
blast.tax <- fread("BF2BR1_CO1.nochim_ASVs_blast_BOLD-ex.txt", header = FALSE)
head(blast.tax)
...
ASVCO1 <- fread("BF2BR1_CO1.nochim_ASVs_counts.tsv", header = T)
head(ASVCO1)
...
#head(ps_tax)
####
sample.data <-fread("C:/Users/joelle.lousberg/OneDrive - Cawthron/Documents/Chapter_2/R_step2/BF2BR1_CO1/CAW-25-36_sample_data.csv")
names(sample.data)
...
phyloseq_BF2BR1 <- phyloseq(ps_otu_table, ps_map, ps_tax)
phyloseq_BF2BR1
...
Controls <- subset_samples(phyloseq1, Sample_type %in% c("Blank"))
  ...
  warning("All control samples have zero reads — skipping taxon filtering.")
} else {
...
phyloseq2 <- subset_taxa(phyloseq1.5, 
                         !Phylum %in% c("d__Eukaryota", "k__Metazoa", "") & 
                           !is.na(Phylum))
phyloseq_BF2BR1_clean <- phyloseq2
saveRDS(phyloseq_BF2BR1_clean, file = "phyloseq_BF2BR1_clean.rds")

##Load from here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
phyloseq2 <- readRDS("phyloseq_BF2BR1_clean.rds")
ntaxa(phyloseq2)
...
#unique(tax_table(phyloseq_zoopl_genus)[, "Genus"])
...
##OLD CODE ____________________________________________________________________________________
...
ggplot(relabund_long_meta, aes(x = Lake, y = RelAbundance*100, fill = Group)) +
  ...
  xlab("Lake") +
  ggtitle("Rarefied & grouped taxa BF2/BR1") +
  theme_minimal() +
  theme(
...
Speciesfig_relzoopl_BF2BR1 <- plot_bar(
  ps_subset_rel,
  x = "Sample_Name",
  fill = "Species"
) +
  ...
  labs(
    title = "Relative Abundance of Species in Branchiopoda, Hexanauplia, and Eurotatoria BF2BR1",
    y = "Relative Abundance [%]",
    x = "Sample"
  )

print(Speciesfig_relzoopl_BF2BR1)



#Save the cleaned phyloseq object for statistical analyses and comparisons
phyloseqBF2BR1 <- phyloseq2
saveRDS(phyloseqBF2BR1, file = "phyloseqBF2BR1.rds")



# Write a csv with relative abundances for species and genus levels for 3 zoopl classes
