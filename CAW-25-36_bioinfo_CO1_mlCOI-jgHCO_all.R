...
#I only want certain samples from my list actually having the mlCOI/jgHCO primers,
#in this case 21,26-32.

# Forward reads
fnFs <- sort(list.files(
  path,
  pattern = "(CAW-25-11-001|CAW-25-11-004|CAW-25-11-017|CAW-25-11-020|CAW-25-11-0(2[2-4])|CAW-25-36-021|CAW-25-36-0(2[6-9]|3[0-2])).*R1_001.fastq.gz$",
  full.names = TRUE
))
...
fnRs <- sort(list.files(
  path,
  pattern = "(CAW-25-11-001|CAW-25-11-004|CAW-25-11-017|CAW-25-11-020|CAW-25-11-0(2[2-4])|CAW-25-36-021|CAW-25-36-0(2[6-9]|3[0-2])).*R2_001.fastq.gz$",
  full.names = TRUE
))
...
path.cut <- file.path(path, "mlCO1_all/cutadapt")
...
# plot the first 5 forward and reverse reads (or fewer if less than 5 exist)
...
jpeg(file="mlCO1_all/mlCO1_all.Quality.Plot.F.jpg",res=300, width=15, height=8, units="in")
fwd_qual_plots
dev.off()

jpeg(file="mlCO1_all/mlCO1_all.Quality.Plot.R.jpg",res=300, width=15, height=8, units="in")
rev_qual_plots
dev.off()
...
##See results from quality plots and trim accordingly
...
out <- filterAndTrim(cutFs, filtpathF, cutRs, filtpathR,
                     truncLen=c(165,165), maxEE=c(4,6), truncQ=2, maxN=0, rm.phix=TRUE,
...
filtpathF2 <- filtpathF[-c(14, 15)] ##remove water blank - no sequences
filtpathR2 <- filtpathR[-c(14, 15)] ##remove water blank - no sequences
...
jpeg(file="mlCO1_all/mlCO1_all.Error.F.Plot.jpg",res=300, width=15, height=8, units="in")
plotErrors(errF, nominalQ=TRUE)
dev.off()

jpeg(file="mlCO1_all/mlCO1_all.Error.R.Plot.jpg",res=300, width=15, height=8, units="in")
...
track <- cbind(out[-c(14, 15), ], sapply(dadaF.pseudo, getN), sapply(dadaR.pseudo, getN), sapply(mergers, getN))
colnames(track) <- c("input", "filtered", "denoisedF", "denoisedR", "merged")
rownames(track) <- sample.names[-c(14, 15)]

write.csv(track,"mlCO1_all/mlCO1_all.track.csv")
...
saveRDS(seqtab, "mlCO1_all/mlCO1_all.seqtab.rds")
...
jpeg(file="mlCO1_all/mlCO1_all.Chim.Dist.Plot.jpg",res=300, width=30, height=8, units="in")
g
dev.off()
...
seqtab2 <- seqtab[,nchar(colnames(seqtab)) %in% seq(310,316)]

seqtab.nochim <- removeBimeraDenovo(seqtab2, multithread=TRUE, verbose=TRUE)
saveRDS(seqtab.nochim, "mlCO1_all/mlCO1_all.seqtab.nochim.rds")

###Produce a table which shows the number or reads at each stage

getN <- function(x) sum(getUniques(x))
track2 <- cbind(out[-c(14, 15), ],sapply(dadaF.pseudo, getN), sapply(dadaR.pseudo, getN), sapply(mergers, getN), rowSums(seqtab.nochim))
colnames(track2) <- c("input", "filtered", "denoisedF", "denoisedR", "merged", "nonchim")
rownames(track2) <- sample.names[-c(14, 15)]

write.csv(track2, "mlCO1_all/mlCO1_all.track2.csv")  
...
seqtab.nochim <- readRDS("mlCO1_all/mlCO1_all.seqtab.nochim.rds")
...
head(asv_fasta)
write.table(asv_fasta, "mlCO1_all/mlCO1_all_ASV_seq_IDs_and_sequences.tsv", sep="\t", quote=F, col.names=NA)
write(asv_fasta, "mlCO1_all/mlCO1_all.nochim_ASVs.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "mlCO1_all/mlCO1_all.nochim_ASVs_counts.tsv", sep="\t", quote=F, col.names=NA)




##blast code - run in HPC terminal 1 after navigating to the correct folder mlCO1_all (Change the query & o)
#blastn -query mlCO1_all.nochim_ASVs.fa -db ~/../../srv/referencedb/COI/COI.NCBI.BOLD.blast.db -out mlCO1_all.nochim_ASVs_blast_BOLD.txt -num_threads 20 -max_target_seqs 10 -perc_identity 70 -qcov_hsp_perc 80
