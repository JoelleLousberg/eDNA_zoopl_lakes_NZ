...
path.cut <- file.path(path, "/mlCO1/cutadapt")
...
jpeg(file="mlCO1/mlCO1.Error.F.Plot.jpg",res=300, width=15, height=8, units="in")
plotErrors(errF, nominalQ=TRUE)
dev.off()

jpeg(file="mlCO1/mlCO1.Error.R.Plot.jpg",res=300, width=15, height=8, units="in")
...
write.csv(out_final, "mlCO1/track.csv")

seqtab <- makeSequenceTable(mergers)

saveRDS(seqtab, "mlCO1/mlCO1.seqtab.rds")
...
jpeg(file="mlCO1/mlCO1.Chim.Dist.Plot.jpg",res=300, width=15, height=8, units="in")
g
dev.off()
...
merged.seqtab <- merged.seqtab[,nchar(colnames(merged.seqtab)) %in% seq(310, 316)]
...
seqtab1 <- readRDS("/srv/users/Joelle/CAW-26-07/mlCO1/mlCO1.seqtab.nochim.rds")
seqtab2 <- readRDS("/srv/users/Joelle/CO1.seqtab.nochim.rds")
seqtab3 <- readRDS("/srv/users/Joelle/CAW-25-36/mlCO1_all/mlCO1_all.seqtab.nochim.rds")

samples_to_remove <- c("CAW-25-11-001", "CAW-25-11-004", "CAW-25-11-017", "CAW-25-11-020", "CAW-25-11-022", "CAW-25-11-023", "CAW-25-11-024")
seqtab3.cleaned <- seqtab3[!rownames(seqtab3) %in% samples_to_remove, ]
merged.seqtab <- mergeSequenceTables(seqtab1, seqtab2, seqtab3.cleaned)


saveRDS(merged.seqtab, "mlCO1/mlCO1.seqtab.nochim_all.rds")

### I won't get a table which shows the number or reads at each stage because I merged to rds runs
...
seqtab.nochim <- readRDS("mlCO1/mlCO1.seqtab.nochim_all.rds")
...
head(asv_fasta)
write.table(asv_fasta, "mlCO1/mlCO1_ASV_seq_IDs_and_sequences.tsv", sep="\t", quote=F, col.names=NA)
write(asv_fasta, "mlCO1/mlCO1.nochim_ASVs.fa")
