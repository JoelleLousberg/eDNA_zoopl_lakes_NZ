...
setwd("C:/Users/joelle.lousberg/OneDrive - Cawthron/Documents/Chapter_2/R_step3/mlCO1")
...
blast.tax <- fread("mlCO1.nochim_ASVs_blast_BOLD-ex.txt", header = FALSE)
head(blast.tax)
...
ASVCO1 <- fread("mlCO1.nochim_ASVs_counts.tsv", header = T)
head(ASVCO1)
...
####
sample.data <-fread("C:/Users/joelle.lousberg/OneDrive - Cawthron/Documents/Chapter_2/R_step3/mlCO1/CAW-26-07_sample_sheet.csv")
names(sample.data)
...
colourCAW <- c(
  "#d45f74", "#fbb15d", "#7f1424", "#00549e", "#9b99cd", "#4ba791", "#ffcc28", "#c3ce58", "#6860a0", "#702365",
  "#4CFFFF", "#acd58e", "#7ac4d3", "#f8d3ca", "#2f4926", "#3398d2", "#4a7637", "#175c7d", "#c3d3c2",
  "#67bd45", "#c45b28", "#e8b5d4", "#85243f", "#76c044", "#2f725e", "#75a54a", "#bc3635", "#205128",
  "#35356d", "#083631", "#fbd872", "#c22c43")
...
####Subtraction of contamination from lab blanks for CAW-25-11 tows
phyloseq_25_11_tow <- subset_samples(
  phyloseq1,
  Run %in% c("mlCO1_25_11_tow", "mlCO1_25_11_Blank")
)

Controls = subset_samples(phyloseq_25_11_tow, Sample_type %in% c("Blank"))
...
Extraction_neg <- subset_samples(phyloseq_25_11_tow, Sample_type %in% c("Blank"))
Extraction_neg
...
Extraction <- as(otu_table(phyloseq_25_11_tow), "matrix")

# Ensure taxa are rows
if (!taxa_are_rows(phyloseq_25_11_tow)) {
...
rownames(Extractiondf) <- taxa_names(phyloseq_25_11_tow)
colnames(Extractiondf) <- sample_names(phyloseq_25_11_tow)
stopifnot(identical(rownames(Extractiondf), taxa_names(phyloseq_25_11_tow)))
stopifnot(identical(colnames(Extractiondf), sample_names(phyloseq_25_11_tow)))
Extraction_OTU <- otu_table(as.matrix(Extractiondf), taxa_are_rows = TRUE)
Tutorial.subtractextract.ps <- phyloseq(
  Extraction_OTU,
  sample_data(phyloseq_25_11_tow),
  tax_table(phyloseq_25_11_tow)
)

org.ss <- as.data.frame(sample_sums(phyloseq_25_11_tow))
...
phyloseq1.5 = subset_samples(phyloseq_25_11_tow, Sample_type != "Blank")

##remove anything not at phylum level
phyloseq2 <- subset_taxa(phyloseq1.5, 
                         !Phylum %in% c("d__Eukaryota", "k__Metazoa", "") & 
                           !is.na(Phylum))
phyloseq_mlCO1_25_11_clean <- phyloseq2
saveRDS(phyloseq_mlCO1_25_11_clean, file = "phyloseq_mlCO1_25_11_clean.rds")


####Subtraction of contamination from lab blanks for CAW-26-07 watersamples
phyloseq_26_07_water <- subset_samples(
  phyloseq1,
  Run %in% c("mlCO1_26_07_water", "mlCO1_26_07_Blank")
)

Controls = subset_samples(phyloseq_26_07_water, Sample_type %in% c("Blank"))
if (sum(sample_sums(Controls)) > 0) {
  Controls <- filter_taxa(Controls, function(x) sum(x) > 0, TRUE)
} else {
  warning("No reads in Controls — skipping filtering step.")
...
#Extract controls from samples
Extraction_neg <- subset_samples(phyloseq_26_07_water, Sample_type %in% c("Blank"))
Extraction_neg
Extraction_neg_max <- apply(as.matrix(otu_table(Extraction_neg)), 1, max)
Extraction_neg_max_vec <- as.vector(Extraction_neg_max)
names(Extraction_neg_max_vec) <- taxa_names(Extraction_neg)  # ensure names stay aligned
Extraction <- as(otu_table(phyloseq_26_07_water), "matrix")

# Ensure taxa are rows
if (!taxa_are_rows(phyloseq_26_07_water)) {
  Extraction <- t(Extraction)
}
Extractiondf <- as.data.frame(Extraction)
Extractiondf <- Extractiondf[names(Extraction_neg_max_vec), , drop = FALSE]
Extractiondf <- sweep(Extractiondf, 1, Extraction_neg_max_vec, "-")
Extractiondf[Extractiondf < 0] <- 0
rownames(Extractiondf) <- taxa_names(phyloseq_26_07_water)
colnames(Extractiondf) <- sample_names(phyloseq_26_07_water)
stopifnot(identical(rownames(Extractiondf), taxa_names(phyloseq_26_07_water)))
stopifnot(identical(colnames(Extractiondf), sample_names(phyloseq_26_07_water)))
Extraction_OTU <- otu_table(as.matrix(Extractiondf), taxa_are_rows = TRUE)
Tutorial.subtractextract.ps <- phyloseq(
  Extraction_OTU,
  sample_data(phyloseq_26_07_water),
  tax_table(phyloseq_26_07_water)
)

org.ss <- as.data.frame(sample_sums(phyloseq_26_07_water))
org.ss$Names <- rownames(org.ss)
new.ss <- as.data.frame(sample_sums(Tutorial.subtractextract.ps))
new.ss$Names <- rownames(new.ss)
control.rem.maxsub <- dplyr::left_join(org.ss, new.ss, by="Names")
...
phyloseq1.5 = subset_samples(phyloseq_26_07_water, Run != "mlCO1_26_07_Blank")

##remove anything not at phylum level
phyloseq2 <- subset_taxa(phyloseq1.5, 
                         !Phylum %in% c("d__Eukaryota", "k__Metazoa", "") & 
                           !is.na(Phylum))
phyloseq_mlCO1_26_07_water_clean <- phyloseq2
saveRDS(phyloseq_mlCO1_26_07_water_clean, file = "phyloseq_mlCO1_26_07_water_clean.rds")



####Subtraction of contamination from lab blanks for CAW-26-07 & CAW-25-36 tows & freezer
phyloseq_26_07_tow <- subset_samples(
  phyloseq1,
  Run == "mlCO1_26_07_tow" |
    Sample_ID %in% c("CAW-26-07-098", "CAW-25-36-029")
)

Controls = subset_samples(phyloseq_26_07_tow, Sample_type %in% c("Blank"))
if (sum(sample_sums(Controls)) > 0) {
  Controls <- filter_taxa(Controls, function(x) sum(x) > 0, TRUE)
} else {
  warning("No reads in Controls — skipping filtering step.")
...
#Extract controls from samples
Extraction_neg <- subset_samples(phyloseq_26_07_tow, Sample_type %in% c("Blank"))
Extraction_neg
Extraction_neg_max <- apply(as.matrix(otu_table(Extraction_neg)), 1, max)
Extraction_neg_max_vec <- as.vector(Extraction_neg_max)
names(Extraction_neg_max_vec) <- taxa_names(Extraction_neg)  # ensure names stay aligned
Extraction <- as(otu_table(phyloseq_26_07_tow), "matrix")

# Ensure taxa are rows
if (!taxa_are_rows(phyloseq_26_07_tow)) {
  Extraction <- t(Extraction)
}
Extractiondf <- as.data.frame(Extraction)
Extractiondf <- Extractiondf[names(Extraction_neg_max_vec), , drop = FALSE]
Extractiondf <- sweep(Extractiondf, 1, Extraction_neg_max_vec, "-")
Extractiondf[Extractiondf < 0] <- 0
rownames(Extractiondf) <- taxa_names(phyloseq_26_07_tow)
colnames(Extractiondf) <- sample_names(phyloseq_26_07_tow)
stopifnot(identical(rownames(Extractiondf), taxa_names(phyloseq_26_07_tow)))
stopifnot(identical(colnames(Extractiondf), sample_names(phyloseq_26_07_tow)))
Extraction_OTU <- otu_table(as.matrix(Extractiondf), taxa_are_rows = TRUE)
Tutorial.subtractextract.ps <- phyloseq(
  Extraction_OTU,
  sample_data(phyloseq_26_07_tow),
  tax_table(phyloseq_26_07_tow)
)

org.ss <- as.data.frame(sample_sums(phyloseq_26_07_tow))
org.ss$Names <- rownames(org.ss)
new.ss <- as.data.frame(sample_sums(Tutorial.subtractextract.ps))
new.ss$Names <- rownames(new.ss)
control.rem.maxsub <- dplyr::left_join(org.ss, new.ss, by="Names")
...
phyloseq1.5 = subset_samples(phyloseq_26_07_tow, Sample_type != "Blank")

##remove anything not at phylum level
phyloseq2 <- subset_taxa(phyloseq1.5, 
                         !Phylum %in% c("d__Eukaryota", "k__Metazoa", "") & 
                           !is.na(Phylum))
phyloseq_mlCO1_26_07_tow_clean <- phyloseq2
saveRDS(phyloseq_mlCO1_26_07_tow_clean, file = "phyloseq_mlCO1_26_07_tow_clean.rds")


####Subtraction of contamination from lab blanks for CAW-25-36 Rototekoiti tow
phyloseq_25_36_tow <- subset_samples(
  phyloseq1,
  Run == "mlCO1_25_36_tow" |
    Sample_ID %in% c("CAW-25-36-029", "CAW-25-36-030")
)

Controls = subset_samples(phyloseq_25_36_tow, Sample_type %in% c("Blank"))
if (sum(sample_sums(Controls)) > 0) {
  Controls <- filter_taxa(Controls, function(x) sum(x) > 0, TRUE)
} else {
  warning("No reads in Controls — skipping filtering step.")
...
#Extract controls from samples
Extraction_neg <- subset_samples(phyloseq_25_36_tow, Sample_type %in% c("Blank"))
Extraction_neg
Extraction_neg_max <- apply(as.matrix(otu_table(Extraction_neg)), 1, max)
Extraction_neg_max_vec <- as.vector(Extraction_neg_max)
names(Extraction_neg_max_vec) <- taxa_names(Extraction_neg)  # ensure names stay aligned
...
phyloseq2 <- subset_taxa(phyloseq1.5, 
                         !Phylum %in% c("d__Eukaryota", "k__Metazoa", "") & 
                           !is.na(Phylum))
phyloseq_mlCO1_25_36_tow_clean <- phyloseq2
saveRDS(phyloseq_mlCO1_25_36_tow_clean, file = "phyloseq_mlCO1_25_36_tow_clean.rds")
...
phyloseq_merged <- merge_phyloseq(phyloseq_mlCO1_26_07_tow_clean, phyloseq_mlCO1_26_07_water_clean, 
                                  phyloseq_mlCO1_25_11_clean, phyloseq_mlCO1_25_36_tow_clean)
saveRDS(phyloseq_merged, file = "phyloseq_mlCO1_26_07_clean.rds")
