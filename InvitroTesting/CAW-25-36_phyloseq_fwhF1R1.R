...
setwd("C:/Users/joelle.lousberg/OneDrive - Cawthron/Documents/Chapter_2/R_step2/fwhF1R1_CO1")
...
blast.tax <- fread("fwhF1R1_CO1.nochim_ASVs_blast_BOLD-1-ex.txt", header = FALSE)
head(blast.tax)
...
ASVCO1 <- fread("fwhF1R1_CO1.nochim_ASVs_counts.tsv", header = T)
head(ASVCO1)
...
phyloseq_fwhF1R1_clean <- phyloseq2
saveRDS(phyloseq_fwhF1R1_clean, file = "phyloseq_fwhF1R1_clean.rds")


##__________________________________________________________________________

phyloseq2 <- readRDS("phyloseq_fwhF1R1_clean.rds")
library(openxlsx)
...
ggplot(relabund_long_meta, aes(x = Lake, y = RelAbundance*100, fill = Group)) +
  ...
  xlab("Lake") +
  ggtitle("Rarefied & grouped taxa fwhF1/R1") +
  theme_minimal() +
  theme(
...
Genusfig_relzoopl_mlCO1 <- plot_bar(
  ...
  labs(
    title = "Relative Abundance of Genera in Branchiopoda, Hexanauplia, and Eurotatoria fwhF1R1",
...
Speciesfig_relzoopl_fwhF1R1 <- plot_bar(
  ps_subset_rel,
  x = "Sample_Name",
  fill = "Species"
) +
  ...
  labs(
    title = "Relative Abundance of Species in Branchiopoda, Hexanauplia, and Eurotatoria fwhF1R1",
    y = "Relative Abundance [%]",
    x = "Sample"
  )

print(Speciesfig_relzoopl_fwhF1R1)
