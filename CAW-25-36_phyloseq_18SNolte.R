...
setwd("C:/Users/joelle.lousberg/OneDrive - Cawthron/Documents/Chapter_2/R_step2/18SNolte_COTS/DADA2_18S_results")
...
###get sample data
sample.data <-fread("C:/Users/joelle.lousberg/OneDrive - Cawthron/Documents/Chapter_2/R_step2/18SNolte_COTS/DADA2_18S_results/CAW-25-36_sample_data.csv")
names(sample.data)
...
phyloseq_18SNolte_clean <- phyloseq2
saveRDS(phyloseq_18SNolte_clean, file = "phyloseq_18SNolte_clean.rds")


phyloseq2 <- readRDS("phyloseq_18SNolte_clean.rds")
ntaxa(phyloseq2)
...
ggplot(relabund_long_meta, aes(x = Lake, y = RelAbundance*100, fill = Group)) +
  ...
  xlab("Lake") +
  ggtitle("Rarefied & grouped taxa 18SNolte_fw/rv") +
  theme_minimal() +
  theme(
...
Genusfig_relzoopl <- plot_bar(
  ...
  labs(
    title = "Relative Abundance of Genera in Branchiopoda, Hexanauplia, and Eurotatoria Uni18S",
...
SpeciesGenusFig_relzoopl <- plot_bar(
  ...
  labs(
    title = "Relative Abundance of Zooplankton 18SNolte (Best Available Taxonomy: Genus/Species)",
    y = "Relative Abundance [%]",
    x = "Sample"
  )
