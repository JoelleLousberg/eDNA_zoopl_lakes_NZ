...
path.cut <- file.path(path, "fwhF1R1_CO1/cutadapt")
...
jpeg(file="fwhF1R1_CO1/fwhF1R1_CO1.Quality.Plot.F.jpg",res=300, width=15, height=8, units="in")
fwd_qual_plots
dev.off()

jpeg(file="fwhF1R1_CO1/fwhF1R1_CO1.Quality.Plot.R.jpg",res=300, width=15, height=8, units="in")
rev_qual_plots
dev.off()
...
out <- filterAndTrim(cutFs, filtpathF, cutRs, filtpathR,
                     truncLen=c(100,100), maxEE=c(4,6), truncQ=2, maxN=0, rm.phix=TRUE,
...
jpeg(file="fwhF1R1_CO1/fwhF1R1_CO1.Error.F.Plot.jpg",res=300, width=15, height=8, units="in")
plotErrors(errF, nominalQ=TRUE)
dev.off()

jpeg(file="fwhF1R1_CO1/fwhF1R1_CO1.Error.R.Plot.jpg",res=300, width=15, height=8, units="in")
...
write.csv(track,"fwhF1R1_CO1/fwhF1R1_CO1.track.csv")
...
saveRDS(seqtab, "fwhF1R1_CO1/fwhF1R1_CO1.seqtab.rds")
...
jpeg(file="fwhF1R1_CO1/fwhF1R1_CO1.Chim.Dist.Plot.jpg",res=300, width=15, height=8, units="in")
g
dev.off()
...
seqtab2 <- seqtab[,nchar(colnames(seqtab)) %in% seq(177, 178)]

seqtab.nochim <- removeBimeraDenovo(seqtab2, multithread=TRUE, verbose=TRUE)
saveRDS(seqtab.nochim, "fwhF1R1_CO1/fwhF1R1_CO1.seqtab.nochim.rds")
...
write.csv(track2, "fwhF1R1_CO1/fwhF1R1_CO1.track2.csv")  
...
seqtab.nochim <- readRDS("fwhF1R1_CO1/fwhF1R1_CO1.seqtab.nochim.rds")
...
head(asv_fasta)
write.table(asv_fasta, "fwhF1R1_CO1/fwhF1R1_CO1_ASV_seq_IDs_and_sequences.tsv", sep="\t", quote=F, col.names=NA)
write(asv_fasta, "fwhF1R1_CO1/fwhF1R1_CO1.nochim_ASVs.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "fwhF1R1_CO1/fwhF1R1_CO1.nochim_ASVs_counts.tsv", sep="\t", quote=F, col.names=NA)

##blast code - run in HPC terminal 1
# blastn -query fwhF1R1_CO1.nochim_ASVs.fa -db ~/../../srv/referencedb/COI/COI.NCBI.BOLD.blast.db -out fwhF1R1_CO1.nochim_ASVs_blast_BOLD.txt -num_threads 20 -max_target_seqs 10 -perc_identity 70 -qcov_hsp_perc 80
